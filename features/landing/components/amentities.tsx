...
const Amentities = () => {
  const RENDER_AMENITIES = amenities.map((a, idx) => {
    return (
      <div
        key={idx}
        className="flex items-center gap-2 bg-[rgba(207,175,143,0.1)] rounded-full py-2 px-4 lg:py-3 lg:px-6 text-[#535353] text-nowrap"
      >
    ...
    <div className="py-[40px] lg:py-[80px]">
      <div className="space-y-4 lg:space-y-6 text-center px-4">
        ...
        <p className="text-[#17173B99] font-poppins capitalize font-normal text-base lg:text-lg tracking-[-0.72px]">
          A self-sustained community designed for everyday comfort:
        </p>
      </div>
      <div className="mt-[24px] lg:mt-[40px]">
        <Slider />
      </div>
      <div className="mt-[24px] lg:mt-[40px] flex flex-wrap items-center justify-center gap-[10px] lg:gap-[20px] max-w-[1226px] mx-auto px-4">
        {RENDER_AMENITIES}
      </div>
    </div>
  );
